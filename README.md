...
notifies: not a subagent finishing, not a tool error, and never a run nobody is sitting
at, whether that is a `claude -p` or an ordinary session a script drives through a pty.
