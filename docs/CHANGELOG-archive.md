...
in private, before the repository was published. They were collected under the version that
happened to be current when the diary stopped, so that heading is a date range rather than a
release. Read it as a build log, not as a list of what shipped in `0.13.0`.

`0.12.0` is a real tag with no section of its own — see the note at the foot of this file.
...
- Verified against a real remote server that produced the bug: the old pid was replaced by a new one.
...
[`configuration.md`](./configuration.md#tls-certificate).
...
No entry was ever written for this release: it predates this changelog. The tag exists and is
listed here only so that no released version is missing from the record.
