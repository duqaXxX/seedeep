...
`commandShape` reads a line with no origin as the user's own keystrokes, so an `origin` that went
away leaves the untagged shape with no gate at all, and any `user` line whose whole content reads
`/name …` becomes a command. No task notification is shaped that way today (0 of 346 measured
2026-08-29), which is what makes this the harder half to notice: it costs nothing until one is.
There is no runtime warning for either half, and there is a reason: neither falls through to
"unknown" anywhere visible. A third kind makes one untagged command read as a prompt, which looks
exactly like a prompt.
