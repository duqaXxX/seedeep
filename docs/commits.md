...
`apps/server/src/core/commit-attribution.ts` (pure), `apps/server/src/server/git.ts` (the git
reads), `apps/server/src/server/transcript-scan.ts` (one cached pass over a transcript, shared with
the tracker-cards feature — see `docs/cards.md`), `apps/server/src/server/session-commits.ts` (the
join), `apps/server/src/client/commits-view.ts` (card + drawer).
...
`rev-list`. No index is taken, and that is enforced rather than assumed — every call carries
...
repo attached, so every repo the corpus knows is asked whether it holds that object, and only the
sessions whose activity brackets the commit are then read.
...
**Endpoint:** `GET /api/commits` — its full contract is in
