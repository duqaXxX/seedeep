...
- The file is found from the launch's task id (a `Monitor` names its own the same way on disk —
  `tasks/<taskId>.output` — so the probe answers for it unchanged), not from the path the
  transcript prints:
...
| `note` | an `attachment` line whose `hook_additional_context` names a `toolUseID`, OR a `<task-notification>` carrying nothing but a `<summary>` | something attached TEXT to the session that seedeep would never have derived — a hook warning about the file just written, a background review reporting findings. `toolUseId` (null when it is about the session and not a call), `hook`, `source` (the writer, when the text declares it as `[from <plugin> plugin]`), `text` (anonymized). **`attachment` is otherwise dropped wholesale** — nearly all of them are the bookkeeping every tool produces, twice per call — and the `toolUseID` is what separates a note from that noise: the 555 SessionStart injections carry none, because they are about the session, not about a call. The `content` is an array of BARE STRINGS, not of `{type:'text'}` blocks, which is why `renderedText` returns nothing for it. Measured 2026-08-10 over 533 sessions: 65 anchored notes in 39 of them, 2 unanchored. Deliberately not modelled as "a security finding": a type keyed on one plugin goes blind the day another one speaks |
