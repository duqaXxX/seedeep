...
It also carries the two facts that say whether anybody is sitting at the session, which a client
that notifies has to read: `entrypoint` (`cli` for an interactive session, `sdk-cli` or `sdk-py`
for a headless one) and `driven`, true when the session was launched in a directory other than the
one its parent process works in, which is what a script driving the TUI through a pty looks like.
`driven` is `null` when the server could not tell, and null means a person: the tray treats a
session as automated only on a positive answer.
