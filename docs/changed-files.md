...
rules. Code: `apps/server/src/core/file-attribution.ts` (pure),
`apps/server/src/core/session-artifacts.ts` (pure), `apps/server/src/server/git.ts` (the git reads),
`apps/server/src/server/transcript-scan.ts` (one cached pass over a transcript, shared with commits
and cards), `apps/server/src/server/session-files.ts` (the join),
`renderFiles`/`openAllFiles` in `apps/server/src/client/graph.ts`.
...
as a side effect and takes `.git/index.lock` to do it. seedeep polls the repository of a session
that is actively committing, and taking that lock — even for a millisecond — can fail the user's own
`git add`/`commit`.
...
**Endpoint:** `GET /api/files` — its full contract is in
